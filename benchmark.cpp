...
void printoutTiming(vector< vector<timingStruct> > times, string filename){
	...
	output << "Frame#\tRead_time\tDetector_time\tExtractor_time\tMatcher_time\tTotal_time\tNum_features(match)\n";
	...
	for (int i = 0; i < 4; i++){
		...
		//	cout << "GPU descriptors size is " << descriptors.cols << " " <<  descriptors.rows << endl;
			...
			//	cout << "matched" << endl;
				...
				num_features = good_matches.size();
...
//	cout << "There are " << good_matches.size() << " good matches." << endl;
	...
	//cout << "Pushing back..." << endl;
		...
		aggregateGPU_times.push_back(GPU_times);
		 ...
		 ss << thresh;
		 string Num = ss.str();	
		string fileName = (string) "gpuFile_" + Num +  (string) ".csv"; 
		printoutTiming(aggregateGPU_times, fileName);
	}
	...
	for (int i = 0; i < 4; i++){
		...
		aggregateCPU_times.push_back(CPU_times);
		 ...
		 ss << thresh;
		 string Num = ss.str();	
		string fileName = (string) "cpuFile_" + Num +  (string) ".csv"; 
		printoutTiming(aggregateCPU_times, fileName);
		
		/*cout << "Total: " << total << " Max: " << max << " Min: " << min << " Average: " 
			<< total / numFrames << " Detector_soonly avg: " << detector_total / numFrames
