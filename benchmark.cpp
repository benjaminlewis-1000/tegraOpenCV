...
//TODO: Decide where to put the start timer.
	for (int i = 0; i < 1; i++){
		...
		vector<float> test;
		int ii = 0;
		gpu::GpuMat descriptors, descriptors_keyframe, keypoints, first_keypoints; // Have to be in scope
		for ( ; ; ){
			ii++;
			capture >> frame;
			if (frame.empty() ){
		...
		cout << "Total: " << total << " Max: " << max << " Min: " << min << " Average: " 
			...
			//::cv::gpu::device::surf::compute_descriptors_gpu(descriptors, kps.ptr<float>(gpu::SURF_GPU::X_ROW), kps.ptr<float>(gpu::SURF_GPU::Y_ROW),
